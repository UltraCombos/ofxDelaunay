...
void Voxelize(const float* vertices, int numVertices, const int* indices, int numTriangleIndices, uint32_t width, uint32_t height, uint32_t depth, uint32_t* volume, ofVec3f minExtents, ofVec3f maxExtents)
{
	memset(volume, 0, sizeof(uint32_t)*width*height*depth);

	// build an aabb tree of the mesh
	AABBTree tree((const ofVec3f*)vertices, numVertices, (const uint32_t*)indices, numTriangleIndices/3);

	// parity count method, single pass
	const ofVec3f extents(maxExtents-minExtents);
	const ofVec3f delta(extents.x/width, extents.y/height, extents.z/depth);
	const ofVec3f offset(0.5f*delta.x, 0.5f*delta.y, 0.5f*delta.z);
	...
	for (uint32_t x=0; x < width; ++x)
	{
		for (uint32_t y=0; y < height; ++y)
		{
			...
			ofVec3f rayDir = ofVec3f(0.0f, 0.0f, 1.0f);
			ofVec3f rayStart = minExtents + ofVec3f(x*delta.x + offset.x, y*delta.y + offset.y, 0.0f);
