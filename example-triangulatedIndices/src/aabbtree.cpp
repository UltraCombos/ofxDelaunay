...
AABBTree::AABBTree(const ofVec3f* vertices, uint32_t numVerts, const uint32_t* indices, uint32_t numFaces)
    : m_vertices(vertices)
    , m_numVerts(numVerts)
    , m_indices(indices)
    , m_numFaces(numFaces)
...
namespace
{
	...
	struct FaceSorter
	{
		FaceSorter(const ofVec3f* positions, const uint32_t* indices, uint32_t n, uint32_t axis)
		...
		{
			const ofVec3f& a = m_vertices[m_indices[face*3+0]];
			const ofVec3f& b = m_vertices[m_indices[face*3+1]];
			const ofVec3f& c = m_vertices[m_indices[face*3+2]];
	...
	inline uint32_t LongestAxis(const ofVec3f& v)
	{    
		if (v.x > v.y && v.x > v.z)
			return 0;
		else
...
void AABBTree::CalculateFaceBounds(uint32_t* faces, uint32_t numFaces, ofVec3f& outMinExtents, ofVec3f& outMaxExtents)
{
	ofVec3f minExtents(FLT_MAX);
	ofVec3f maxExtents(-FLT_MAX);

    // calculate face bounds
    for (uint32_t i=0; i < numFaces; ++i)
    {
        ofVec3f a = ofVec3f(m_vertices[m_indices[faces[i]*3+0]]);
        ofVec3f b = ofVec3f(m_vertices[m_indices[faces[i]*3+1]]);
        ofVec3f c = ofVec3f(m_vertices[m_indices[faces[i]*3+2]]);
		
        minExtents = vec3_min(a, minExtents);
        maxExtents = vec3_max(a, maxExtents);

        minExtents = vec3_min(b, minExtents);
        maxExtents = vec3_max(b, maxExtents);

        minExtents = vec3_min(c, minExtents);
        maxExtents = vec3_max(c, maxExtents);
    }
...
bool AABBTree::TraceRay(const ofVec3f& start, const ofVec3f& dir, float& outT, float& u, float& v, float& w, float& faceSign, uint32_t& faceIndex) const
{   
    //s_traceDepth = 0;

	ofVec3f rcp_dir(1.0f/dir.x, 1.0f/dir.y, 1.0f/dir.z);
...
void AABBTree::TraceRecursive(uint32_t nodeIndex, const ofVec3f& start, const ofVec3f& dir, float& outT, float& outU, float& outV, float& outW, float& faceSign, uint32_t& faceIndex) const
{
    ...
    else
    {
        ...
        for (uint32_t i=0; i < node.m_numFaces; ++i)
        {
            ...
            const ofVec3f& a = m_vertices[m_indices[indexStart+0]];
            const ofVec3f& b = m_vertices[m_indices[indexStart+1]];
            const ofVec3f& c = m_vertices[m_indices[indexStart+2]];
#if TRACE_STATS
            extern uint32_t g_trisChecked;
            ++g_trisChecked;
#endif
...
*/
bool AABBTree::TraceRaySlow(const ofVec3f& start, const ofVec3f& dir, float& outT, float& outU, float& outV, float& outW, float& faceSign, uint32_t& faceIndex) const
{    
	...
	ofVec3f minNormal(0.0f, 1.0f, 0.0f);

	ofVec3f n(0.0f, 1.0f, 0.0f);
    float t, u, v, w, s;
    bool hit = false;
	uint32_t minIndex = 0;
    ...
    for (uint32_t i=0; i < numFaces; ++i)
    {
        const ofVec3f& a = m_vertices[m_indices[i*3+0]];
        const ofVec3f& b = m_vertices[m_indices[i*3+1]];
        const ofVec3f& c = m_vertices[m_indices[i*3+2]];
